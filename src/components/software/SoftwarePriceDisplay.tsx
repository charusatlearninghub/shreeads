...
import { useActivePromotionForSoftware } from '@/hooks/usePromotions';
import { formatPrice, calculateDiscountPercentage } from '@/lib/price-utils';
import { formatDistanceToNow, parseISO } from 'date-fns';

interface SoftwarePriceDisplayProps {
  productId: string;
  price: number | null;
...
export const SoftwarePriceDisplay = ({
  productId,
  price,
  discountPrice,
  isFree,
  showPromotion = true,
  size = 'md',
}: SoftwarePriceDisplayProps) => {
  const { data: activePromotion } = useActivePromotionForSoftware(productId);
